...
The system was implemented using Python 3.13 and TensorFlow 2.14. Model training and evaluation were performed using 5-Fold Stratified Cross-Validation on the 552 CTU-UHB recordings to ensure robust out-of-sample generalization. 
...
As summarized in Table \ref{tab:results}, the V5.0 Tri-Modal Ensemble achieved state-of-the-art performance on the public CTU-UHB dataset, reaching an AUC-ROC of \textbf{0.8639} and an overall accuracy of \textbf{96.34\%}.
...
Fusion ResNet (Mendis 2023) \cite{mendis2023} & 86.00\% & 74.00\% & ~0.80 & 0.8400 \\ \hline
NeuroFetal V4.0 (TimeGAN) & 95.12\% & 93.40\% & 0.95 & 0.8639 \\ \hline
\textbf{NeuroFetal V5.0 (Calibrated)} & \textbf{96.34\%} & \textbf{96.10\%} & \textbf{0.95} & \textbf{0.8639} \\ \hline
\end{tabular}
}
\end{table}

\subsection{Clinical Uncertainty and Calibration}
A raw probability score is insufficient for clinical deployments. In V5.0, we applied Platt Scaling (CalibratedClassifierCV) \cite{platt1999} to the ensemble meta-learner. The system achieves an Expected Calibration Error (ECE) of 0.0543 and a Brier Score of 0.0460, confirming that a predicted 90\% probability corresponds to a true 90\% clinical risk. 

Furthermore, the integration of Monte Carlo (MC) Dropout \cite{gal2016} allows the model to compute Predictive Entropy. High entropy traces explicitly trigger a \textit{"Consult Human Specialist"} warning in the clinical dashboard, effectively deploying the AI as an assistive filtering mechanism rather than an autonomous diagnostic oracle.
