...
presents \textbf{NeuroFetal AI v6.0}, a clinically
...
signals, and 18-dimensional maternal clinical tabular features. The core
...
trained on 37 hand-crafted tabular and Common Spatial Pattern (CSP) features.
...
pathological) without introducing data leakage, we execute a \textit{TimeGAN}-based augmentation
strategy—a Wasserstein GAN with gradient penalty (WGAN-GP)—strictly within the cross-validation
folds (per-fold training). This synthesizes physiologically coherent
minority-class recordings that preserve temporal dynamics such as
late decelerations and contraction timing, exclusively from training data. Model calibration is performed via
...
cross-validation, NeuroFetal AI v6.0 achieves an \textbf{Ensemble AUC of 0.8566 and an Expected Calibration Error (ECE) of 0.2417}, surpassing
the state-of-the-art baseline of Mendis et al.\ (whose architecture independently reproduced an AUC of 0.7983 on our identical processing pipeline), which originally relied on an
...
resulting from insufficient uteroplacental oxygen delivery. Catching fetal
compromise early and accurately is essential because it gives doctors time
to intervene—whether through an emergency cesarean section or instrumental
delivery. Such timely actions are often the only way to prevent fetal death
or permanent neonatal brain damage.

\subsection{Cardiotocography: The Clinical Standard and Its Limitations}

hardware concurrently records the Fetal Heart Rate (FHR) through a Doppler 
ultrasound belt, while simultaneously measuring Uterine Contractions (UC) 
with a pressure tocodynamometer. 
...
\begin{enumerate}
    \item To architect a tri-modal fusion mechanism capable of extracting 
    and combining the distinct, complementary patterns hidden within FHR 
    waves, UC contraction cycles, and static bedside clinical features to 
    better classify fetal compromise.
...
consistently outperform ResNets on the CTU-UHB benchmark.
...
\begin{enumerate}
    ...
    this on the public CTU-UHB dataset alone (AUC 0.8566), establishing a
    reproducible and defensibly leak-free benchmark.
\end{enumerate}
...
\textbf{NeuroFetal AI v6.0} & \textbf{0.8566} & \textbf{\cmark} & \textbf{\cmark} & \textbf{\cmark} & \textbf{\cmark} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\subsection{Source: CTU-UHB Intrapartum CTG Database}
...
\midrule
\multirow{4}{*}{Parity ($n$ prev.\ births)} 
        & 0      & 376 (68.1\%) & & \\
        & 1      & 140 (25.4\%) & & \\
        & 2      & 29 (5.2\%) & & \\
        & $\geq 3$& 7 (1.3\%) & & \\
...
For each 20-minute window, 18 structured clinical features were extracted
...
\textbf{Signal-Derived Traits (15)}: Per-window metrics extracted straight from the FHR waveform:
\begin{enumerate}
    ...
    \item \textit{Baseline Variance} — Variance of the estimated baseline across the 20-minute window.
    \item \textit{Deceleration Area} — Total integrated area of deceleration dips below the baseline.
\end{enumerate}
...
\texttt{X\_tabular.npy} & $(N, 18)$      & 3 demographic + 15 signal-derived \\
...
augmentation strategy. NeuroFetal AI v6.0 transitioned from a globally trained
SMOTE-based augmentation to \textbf{Per-Fold TimeGAN}, a time-series generative adversarial network
\cite{yoon2019} implemented as a WGAN-GP architecture to strictly prevent data leakage:

\begin{itemize}
    \item \textbf{Generator Architecture}: 1D Transposed Convolution network
    trained \textit{exclusively} on the pathological FHR+UC paired windows found within the $\frac{4}{5}$ training split of each cross-validation fold.
    \item \textbf{Training Signal}: Wasserstein distance with gradient penalty
    (WGAN-GP, $\lambda = 10$) for training stability over 1,500 epochs per fold.
    \item \textbf{Data Leakage Prevention}: Unlike early iterations (V4.0/V5.0), where TimeGAN was trained globally on all pathological data (inadvertently creating overlapping synthetics that leaked validation patterns into training sets), the V6.0 TimeGAN is rigorously refitte on each CV fold. 
    \item \textbf{Temporal Fidelity}: TimeGAN preserves
...
Following the per-fold TimeGAN augmentation, standard time-series jitter, scaling,
...
\node[data_box, below=1cm of csp] (tab) {Clinical Context \\ $\mathbf{X}_{\text{tab}} \in \mathbb{R}^{18}$};
